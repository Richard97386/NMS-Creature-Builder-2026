...
export const BuilderPage: React.FC = () => {
  ...
  const [saveJson, setSaveJson] = useState<CreatureSave>(defaultPetJson(false));
  ...
  const onChangeCreatureDropDown = (e: any) => {
   ...
   const modifyJsonObj = (name: string, value: any) => {
    ...
    // 🎯 THE LIVE COUPLER TRIGGER: Forces your visual JSON text preview window 
    // to recalculate and display your new slider or name values instantly as you type!
    triggerJsonInterval();
  };
  ...
  const getDescriptorValue = (): Array<string> => {
    ...
    return descriptors.filter(d => d !== noDescriptorOptionKey);
     ...
     // 🎯 UNIVERSAL FIELD OVERWRITE: Merges names, scales, trust, and traits live into the copy box!
    const getJsonFromMappings = (localMappingString: string): string => {
    if (selectedPet.CreatureId == null) return '';
    
    const descriptors: Array<string> = localMappingString
      .split(',')
      .filter(descr => descr.length > 0 && descr !== noDescriptorOptionKey);
      
    const displayDescrips = descriptors.map(descr => `^${descr}`);

    // Automatically append the tracking tag for community validation format
    if (!displayDescrips.some(d => d.includes('NOMANSAPP.COM'))) {
      displayDescrips.push('^NOMANSAPP.COM');
    }

    // 🎯 THE FIX: Instead of resetting to template defaults, we spread "...saveJson"
    // to preserve every slider value, text input, and trait modification instantly!
    const finalObj = {
      ...saveJson,
      "CreatureID": `^${selectedPet.CreatureId}`,
      "Descriptors": displayDescrips,
      "PetBattlerUseCoreStatClassOverrides": settings.useCoreStatClassOverrides ?? saveJson.PetBattlerUseCoreStatClassOverrides ?? false
    };
  ...
  return (
    <BasePage>
      ...
      <Box className={creatureIdIsNotNull ? 'attributes' : 'hidden'}>
        ...
        />
            {/* 🎯 THE COMPOSITE DASHBOARD ROW: Splits your layout into the 3D canvas box, JSON text area, and part drop-downs */}
      <Flex className="builder-page-flex" direction={{ base: 'column', xl: 'row' }} gap="4" mt="4">
        
        {/* 1. 🎯 RESTORED 3D VIEWPORT CONTAINER: Forces the canvas area to maintain visible dimensions */}
        {settings.showModelPreview && (
          <Box 
            ...
            zIndex="1"
          >
            {/* Renders the active Three.js 3D viewport canvas natively */}
            <ObjViewer 
              creatureId={selectedPet.CreatureId || ""} 
              meshesToHide={mappingString} 
              cameraPositionZ={10}
            />
          </Box>
        )}
        ...
        {/* 2. 🎯 THE GENERATED JSON TEXT OUTPUT CONTAINER */}
        {settings.showJsonPreview && (
          <Box flex="4" minW={{ base: '100%', xl: '350px' }} height="450px">
        ...
        {/* 3. THE LIVE SELECTORS & DROPDOWN PARAMETERS NAVIGATION MATRIX */}
        <Box flex="4" className="builder-controls" bg="gray.800" p="4" borderRadius="md" border="1px solid" borderColor="gray.700">
          <DescriptorSelector 
            creatureId={selectedPet.CreatureId} 
